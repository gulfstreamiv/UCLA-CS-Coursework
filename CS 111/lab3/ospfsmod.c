...
*/
			...
			ok_so_far = filldir(dirent, od->od_name, sizeof(od->od_name),f_pos, dir_inode->i_ino, file_type);
	...
	while (amount < count && retval >= 0) 
{
	...
	{ //status == 0 success copying data from disk into buffer
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	ospfs_direntry_t* emptyDirEntry = create_blank_direntry(ospfs_inode(dir->i_ino));
	if(IS_ERR(emptyDirEntry))
		return PTR_ERR(emptyDirEntry);
	...
	emptyDirEntry->od_ino = i;
	memcpy(emptyDirEntry->od_name, dentry->d_name.name, dentry->d_name.len);
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	char* beginPos = strpbrk(symname, "?");
	char* endPos = strpbrk(symname, ":");
	int if_conditional_symlink = beginPos && endPos;

	if(dentry->d_name.len > OSPFS_MAXNAMELEN ||
		!if_conditional_symlink && strlen(symname) > OSPFS_MAXSYMLINKLEN ||
		if_conditional_symlink && strlen(symname) + (endPos - beginPos) > OSPFS_MAXSYMLINKLEN)
		return -ENAMETOOLONG;
	...
	ospfs_symlink_inode_t *linkPtr = ospfs_inode(entry_ino); //create sym link pointing to the created file

	linkPtr->oi_size = strlen(symname);
	linkPtr->oi_ftype = OSPFS_FTYPE_SYMLINK;
	memcpy(linkPtr->oi_symlink, symname, strlen(symname));

	// if its a conditional symlink, duplicate path 1 at the end of the symname
	if(if_conditional_symlink)
		memcpy(linkPtr->oi_symlink+strlen(symname)+1, beginPos+1, endPos-beginPos-1);
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	// check if it's a conditional symlink
	char* beginPos = strpbrk(symlink, "?");
	char* endPos = strpbrk(symlink, ":");
	int if_conditional = beginPos && endPos;

	if(if_conditional)
	{
		if(current->uid) // NOT root
			nd_set_link(nd, endPos+1);
		else // root
			nd_set_link(nd, symlink+strlen(symlink)+1);
	}
	else
