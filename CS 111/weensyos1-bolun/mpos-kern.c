...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_WAIT: {
	  ...
	  pid_t processID = current->p_registers.reg_eax;
	  if (processID <= 0 || processID >= NPROCS || processID == current->p_pid
	      || proc_array[processID].p_state == P_EMPTY)
	    current->p_registers.reg_eax = -1;
	  else if (proc_array[processID].p_state == P_ZOMBIE){
	    current->p_registers.reg_eax = proc_array[processID].p_exit_status;
	    current->waitingIndex =-1;
	    current->p_state = P_RUNNABLE;
	    proc_array[processID].p_state = P_EMPTY;
	  }else{
	    current->p_state  = P_BLOCKED;
	    current->waitingIndex = processID;
	  }
	  schedule();
	}
...
do_fork(process_t *parent)
{
  ...
  int procID = 1;
  for (; procID < NPROCS; procID++)
    if(proc_array[procID].p_state == P_EMPTY)
      break;
  
  if (procID == NPROCS)
    return -1;

  proc_array[procID].p_registers = parent->p_registers;

  copy_stack(&proc_array[procID],parent);
  proc_array[procID].p_state = P_RUNNABLE;
  proc_array[procID].p_registers.reg_eax = 0;
	...
	while (1) 
	  {
	  	...
	  	if (proc_array[pid].p_state == P_RUNNABLE)
	    	run(&proc_array[pid]);
		   	...
		   	proc_array[proc_array[pid].waitingIndex].p_state  == P_ZOMBIE)
	    	{
	      	   proc_array[pid].p_registers.reg_eax = 
	           proc_array[proc_array[pid].waitingIndex].p_exit_status;
	           run(&proc_array[pid]);
	    	}
	  }
}
