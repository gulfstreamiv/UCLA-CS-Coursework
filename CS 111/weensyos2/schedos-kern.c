...
schedule(void)
{
	...
	pid_t cur_proc;
	pid_t to_be_run_process = 8888;
	pid_t i;
	int current_high_priority;
	int current_priority;
	float cur_high_proportion = 8888;
	...
	else if(scheduling_algorithm == 2)
	{
	  ...
	  current_high_priority = 8888;
	  for(i = 1;i <=NPROCS;i++)
	  {
	    cur_proc = (i + pid)% NPROCS;
	    current_priority = proc_array[cur_proc].p_priority;
	    if(proc_array[cur_proc].p_state == P_RUNNABLE 
	       && current_priority < current_high_priority)
		{
	      current_high_priority = current_priority;
	      to_be_run_process = cur_proc;
	    }
	  }
	  if(to_be_run_process != 8888)
	    run(&proc_array[to_be_run_process]);
	}
	else if(scheduling_algorithm ==3)
	{
	  ...
	  for(i = 1; i <=NPROCS; i++){
	    cur_proc = (i + pid)% NPROCS;
	    current_proportion = (float)proc_array[cur_proc].p_time_executed/(float)proc_array[cur_proc].p_proportion;
	    	    
	    if(proc_array[cur_proc].p_state == P_RUNNABLE && 
	       current_proportion <= cur_high_proportion){
	      cur_high_proportion = current_proportion;
	      to_be_run_process = cur_proc;
	    }
	  }
	  if(to_be_run_process != 8888){
	    proc_array[to_be_run_process].p_time_executed ++;
	    run(&proc_array[to_be_run_process]);
	  }
	}
	else if(scheduling_algorithm ==4)
	{
	  ...
	  while(1){
	    j++;
	    to_be_run_process = select_a_ticket();
	    if(proc_array[to_be_run_process].p_state == P_RUNNABLE)
	      run(&proc_array[to_be_run_process]);
	    if (j > 10){
