 ...
 * IMPORTANT NOTE
 * Liem Do
 * The first page of the index file is used to store rootPid and treeHeight
 */
...
class BTreeIndex {
 public:
   ...
   */
  RC pointToSmallestKey(IndexCursor& cursor);
 ...
 private:
	 char* treeInfo_buffer[PageFile::PAGE_SIZE]; // mainly used to store the pagePid = 0, which contains
	 ...
	 */
	 RC insertHelper(PageId parentNode, int currentLevel, PageId currentNode, const int &key, const RecordId &rid);
