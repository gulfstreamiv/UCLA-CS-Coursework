...
float prologue = 10.025; // SET THIS TO ZERO
float timeline = 33; // SET THIS TO ZERO
float rotation = 0;
float rotateX = 0;
float rotateZ = 0;
float grassSway = 0;
...
void drawBFMHearts(void)
{
	mvstack.push(model_view);
	set_colour(1.25,0,2.55);
		if(timeline > 56)
			model_view *= RotateY(-90);

		if((timeline > 38.5f && timeline < 39.0f)) {
			mvstack.push(model_view);
				model_view *= Translate(0.0f, 2.0f, 0.0f);
				drawLoveHeart();
			model_view = mvstack.pop();
		}
		if((timeline > 38.75f && timeline < 39.25f)) {
			mvstack.push(model_view);
				model_view *= Translate(1.0f, 2.0f, 0.0f);
				model_view *= RotateZ(-15);
				drawLoveHeart();
		...
		if((timeline > 39.0f && timeline < 39.5f) ) {
			mvstack.push(model_view);
				model_view *= Translate(-2.0f, 2.5f, 0.0f);
				model_view *= RotateZ(15);
				drawLoveHeart();
		...
		if((timeline > 39.25f && timeline < 39.75f)) {
			mvstack.push(model_view);
				if(timeline > 56.75)
					model_view *= Translate(0.0f, 3.0f+temp3, 0.0f);
				else
...
// draw GFS
void drawGFS(float r, float g, float b)
{
	mvstack.push(model_view);
	    model_view *= Translate(0.0f, 1.0f, 0.0f);
		//draw head
		...
		set_colour(r, g, b);
		model_view *= Translate(0.0f, -2.0f, 0.0f);
		model_view *= Scale(1.2f, 1.21f, 1.2f);
		model_view*=RotateX(90);
		drawCylinder();
	...
	mvstack.push(model_view);
		...
		if(timeline > 30.5) {
			model_view *= Translate(-6.0f, 0.5f, 0.5f);
	...
	//draw leg standing
	if(timeline >= 28.0f && timeline < 50.0f) {
		set_colour(r,g,b);
		model_view *= Translate(0.5f, -2.6f, 0.0f);
		mvstack.push(model_view);
			if(timeline < 36.0||timeline >40.0)
				model_view *= RotateX(moveLegs);
