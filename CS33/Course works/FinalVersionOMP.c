...
gaussian_blur(float **src, float **dst, int w, int h, float sigma)
{
	...
	int a;	
	for(a=0;a<tmp2;a++)
	ringbuf[a] = malloc(ksize * sizeof(float));
#pragma omp parallel
	{
	...
	for (y = 0; y < h; y++)
	{
		int bufi0 = ksize-1;
		float tmp = src[y][0];
		for (x = 0; x < halfk  ; x++) ringbuf[y][x] = tmp;
		for (     ; x < ksize-1; x++) ringbuf[y][x] = src[y][x-halfk];

		for (x = 0; x < xmax; x++)
		{
			ringbuf[y][bufi0++] = src[y][x+halfk];
			if (bufi0 == ksize) bufi0 = 0;
			dst[y][x] = convolve(kernel, ringbuf[y], ksize, bufi0);
		}
	...
	for (x = 0; x < w; x++)
	{
		int bufi0 = ksize-1;
		float tmp = dst[0][x];
		for (y = 0; y < halfk  ; y++) ringbuf[x][y] = tmp;
		for (     ; y < ksize-1; y++) ringbuf[x][y] = dst[y-halfk][x];

		for (y = 0; y < ymax; y++)
		{
			ringbuf[x][bufi0++] = dst[y+halfk][x];
			if (bufi0 == ksize) bufi0 = 0;

			dst[y][x] = convolve(kernel, ringbuf[x], ksize, bufi0);
		}
